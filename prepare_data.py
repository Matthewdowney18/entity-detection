...
"""
goal: create a json file with training data from conll files.
...
sample data
{
    file_id: {
        sent_id: [
                ...
                'word_nb': word number,
                'speaker': spealer,
                'iob_labels': (I, O1, O2, O3, B1, B2, B3),
                'inside_ids': [list of corererence ids]],
                'num_inside': number of different mentions this word is apart of,
                'o_b_ids': (O1, O2, O3, B1, B2, B3)},
                {"word": word, "labels": (I, O1, O2, O3, B1, B2, B3)},
                {"word": "Hello", "labels": (1, 0, 1, 0, 0, 0, 0)}
            ],
        1:  []
        "len" : 2
        }
...
include beginning of sentence, seperator tokens. not end of sentence, because it may
need pruneing.
"""

def fix_tuple(tuple):
    """
    the goal of this function is to push the value in the tuple to the front,
    and leave the zeros at the end

    ex: (1, 0, 1) -> (1, 1, 0)
        (0, 0, 35) -> (35, 0, 0)
        (0, 3, 9) -> (3, 9, 0)

    actually never used this but its nice lol. probably a better way to do this
    still lol
    :param tuple:
    :return:
    """
